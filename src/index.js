...
const Field = ({ tagName, rule, name, label, props }) => {
  const TargetComponent = get(fields, tagName);
...
const mapping = {
  ...
  submitButton: ({ children, props }) => (
    <SubmitButton block type="primary" {...props}>
      {children || '提交'}
    </SubmitButton>
  ),
  resetButton: ({ children, props }) => (
    <ResetButton block {...props}>
      {children || '重置'}
    </ResetButton>
  )
};
...
const FormRender = ({ schema: _schema, ...props }) => {
  ...
  return (
    <Form {...props} className={classnames('form-render', schema.className)} type={schema.type} scrollToError={schema.scrollToError} enterSubmit={schema.enterSubmit} size={schema.size}>
      <div
        style={{
    ...
    <ErrorBoundary
      errorRender={error => {
        ...
        if (Array.isArray(error) && error[0] instanceof ValidationError) {
          return <Alert message="表单渲染失败" description={error.map(item => `${item.message}，请检查「${JSON.stringify(item.instance)}」处数据是否符合jsonschema规则`).join(',')} type="error" />;
        }
        return null;
      }}>
      <FormRender {...props} />
